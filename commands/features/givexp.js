...
async function giveGodpower(message, userData, client) {
    ...
    if (nextLevel <= newGodpower) {
        ...
        const lvlUpEmbed = new EmbedBuilder()
            ...
            .addFields([{ name: 'Gold rewarded', value: `You earned ${goldAdd} <:stat_gold:401414686651711498> for reaching level ` + User[message.author.id].level + '. You now have ' + User[message.author.id].gold + ' gold total.' }])

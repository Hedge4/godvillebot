...
async function accept(message, client, Discord, userData) {
    ...
    new_channel.send(`${author} accepted :white_check_mark: a suggestion by ${username}:`, { embed: acceptedEmbed });
    botChannel.send(`${author} accepted :white_check_mark: a suggestion by ${username}:`, { embed: acceptedEmbed });
}
...
async function reject(message, client, Discord) {
    ...
    const botChannel = client.channels.cache.get(botvilleChannel);
    new_channel.send(`${author} rejected :x: a suggestion by ${username}:`, { embed: acceptedEmbed });
    botChannel.send(`${author} rejected :x: a suggestion by ${username}:`, { embed: acceptedEmbed });
}
